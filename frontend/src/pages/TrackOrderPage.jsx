...
function TrackOrderPage() {
  ...
  const handleGetOrder = useCallback(async () => {
    try {
      const result = await axios.get(`${serverUrl}/api/order/${orderId}`, {
        withCredentials: true,
      });
      setCurrentOrder(result.data);

      const so0 = result.data?.shopOrders?.[0];
      if (so0?.deliveryOtp) {
        setDeliveryOtp(String(so0.deliveryOtp));
        setOtpExpiresAt(so0.otpExpires ? new Date(so0.otpExpires) : null);
      }
  ...
  const handleRetryBroadcast = async () => {
    ...
    try {
      await axios.post(
        `${serverUrl}/api/order/retry-broadcast`,
        { orderId },
        { withCredentials: true },
      );
  ...
  useEffect(() => {
    if (socket && orderId) {
      ...
      const handleDeliveryOtp = (payload) => {
        if (payload?.orderId !== orderId) return;
        if (payload?.otp) setDeliveryOtp(String(payload.otp));
        if (payload?.expiresAt) setOtpExpiresAt(new Date(payload.expiresAt));
      };

      socket.on("deliveryLocationUpdate", handleUpdateLocation);
      socket.on("orderStatusUpdated", handleStatusUpdate);
      socket.on("orderAccepted", handleOrderAccepted);
      socket.on("deliveryOtp", handleDeliveryOtp);

      return () => {
        socket.off("deliveryLocationUpdate", handleUpdateLocation);
        socket.off("orderStatusUpdated", handleStatusUpdate);
        socket.off("orderAccepted", handleOrderAccepted);
        socket.off("deliveryOtp", handleDeliveryOtp);
      };
    }
  ...
  const orderStatusLabel =
    String(orderStatus).toLowerCase() === "delivered"
      ...
      : orderStatus;
  const otpIsExpired =
    otpExpiresAt instanceof Date && !Number.isNaN(otpExpiresAt.getTime())
      ? otpExpiresAt.getTime() < Date.now()
      : false;
  const showOtp = Boolean(deliveryOtp) && !otpIsExpired;
  ...
  return (
    <div className='min-h-screen bg-[#fffcf8] pb-12 font-["Outfit"]'>
      <div className="max-w-5xl mx-auto px-4">
        {/* 🔙 BACK NAV */}
        <div
          className="flex items-center gap-4 py-8 cursor-pointer group"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-orange-100/50 overflow-hidden border-8 border-white h-125 relative">
              {liveLocation ? (
                <DeliveryBoyTracking
                ...
                <div className="w-full h-full bg-gray-50 flex flex-col items-center justify-center p-12 text-center">
                  ...
                  <p className="text-gray-400 font-medium max-w-xs mb-6">
                    We're finding the best delivery partner for your order. Hang
                    tight!
                  </p>

                  <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 max-w-sm">
                    <p className="text-orange-800 font-bold text-sm mb-4">
                      Broadcast Failed (No Delivery Boy Found)
                    </p>
                    <button
                    ...
                    {retryMessage && (
                      <p className="mt-3 text-xs font-bold text-orange-600 uppercase tracking-widest">
                        {retryMessage}
                      </p>
                    )}
                    <p className="text-[10px] text-gray-400 font-medium mt-4 italic">
                      Waiting for a delivery boy to accept the order...
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="absolute top-6 left-6 right-6 z-1000">
                <div className="bg-white/90 backdrop-blur-md p-4 rounded-2xl shadow-xl border border-white flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full animate-pulse ${orderStatus === "delivered" ? "bg-green-500" : "bg-[#ff4d2d]"}`}
                    ></div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-black text-[#ff4d2d]">
                      15-20 MINS
                    </p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            {showOtp && (
              <div className="bg-white rounded-4xl p-6 shadow-xl border border-gray-100">
                ...
                <p className="text-3xl font-black text-[#ff4d2d] tracking-[0.35em]">
                  {deliveryOtp}
                </p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-gray-900">
                      {currentOrder.deliveryBoy.fullName}
                    </h4>
                  ...
                  <a
                    href={`tel:${currentOrder.deliveryBoy.mobile}`}
                    className="ml-auto bg-gray-900 text-white p-3 rounded-xl hover:scale-110 transition-transform"
                  >
                    <MdPhone size={20} />
                  </a>
                </div>
              ...
              <div className="space-y-3">
                {currentOrder.shopOrders.map((so) =>
                  so.shopOrderItems.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center bg-[#fffcf8] p-3 rounded-xl border border-orange-50"
                    >
                      <span className="text-sm font-bold text-gray-700">
                        {item.name || item.item?.name || "Item"}{" "}
                        <span className="text-[#ff4d2d]">x{item.quantity}</span>
                      </span>
