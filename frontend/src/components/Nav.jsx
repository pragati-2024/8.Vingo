    ...
    <div className="w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-white/10 backdrop-blur-md border-b border-white/10 overflow-visible">
      ...
      <div className="flex items-center gap-4">
        ...
        {userData?.role == "owner" ? (
          <>
            <button
              className="hidden md:flex items-center gap-1 p-2 cursor-pointer rounded-full bg-green-500/10 text-green-600 font-medium"
              onClick={() => {
          ...
          <>
            {userData?.role == "user" && (
              ...
              >
                <FiShoppingCart
                  size={25}
                  className="text-[#ff4d2d] group-hover:drop-shadow-lg transition-all"
                />
                <AnimatePresence>
                  {cartItems.length > 0 && (
                    <Motion.span
