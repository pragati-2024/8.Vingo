        ...
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            ...
            {data.shopOrders.map((shopOrder, index) => (
                <div className='border rounded-lg p-3 bg-[#fffaf7] space-y-3' key={index}>
                    <p className='font-bold text-gray-800'>{shopOrder.shop?.name || 'Shop Name'}</p>
                    ...
                    <div className='flex space-x-4 overflow-x-auto pb-2'>
                        {shopOrder.shopOrderItems.map((item, index) => (
                            <div key={index} className='flex-shrink-0 w-40 border rounded-lg p-2 bg-white'>
                                ...
                                {shopOrder.status == "delivered" && item.item?._id && <div className='flex space-x-1 mt-2'>
                                    {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    <div className='flex justify-between items-center border-t pt-2'>
                        ...
                        <span className='text-sm font-black uppercase text-[#ff4d2d]'>{shopOrder.status}</span>
                    </div>
                </div>
            ))}
