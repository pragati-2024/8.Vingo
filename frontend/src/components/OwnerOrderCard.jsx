...
import { removeMyOrder, updateRealtimeOrderStatus } from "../redux/userSlice";
  ...
  const handleUpdateStatus = async (orderId, shopId, status) => {
    try {
      const result = await axios.put(
        `${serverUrl}/api/order/status/${orderId}/${shopId}`,
        { status },
    ...
    <div className="bg-white rounded-xl shadow-md p-5 space-y-4 border border-gray-100 hover:shadow-lg transition-shadow">
      ...
      <div className="flex space-x-4 overflow-x-auto pb-2">
        {data.shopOrder?.shopOrderItems?.map((item, index) => (
          <div
            key={index}
            className="flex-shrink-0 w-32 border border-gray-100 rounded-lg p-2 bg-white"
          >
            <img
              src={
                resolveMediaUrl(item.item?.image) ||
      ...
      <div className="flex justify-between items-center pt-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-sm font-bold px-2 py-1 rounded-full uppercase ${
          ...
          >
            {formatStatus(data.shopOrder?.status)}
          </span>
        </div>
        ...
        {data.shopOrder?.status !== "delivered" && (
          <select
            ...
            onChange={(e) =>
              handleUpdateStatus(data._id, shopId, e.target.value)
            }
          >
          ...
          <p className="font-bold text-orange-800 mb-2 flex justify-between items-center">
            ...
            {!data.shopOrder?.assignedDeliveryBoy &&
              (availableBoys.length > 0 ||
                data.shopOrder?.availableBoys?.length > 0) && (
                <button
                  onClick={async () => {
                    const boys =
                      availableBoys.length > 0
                        ? availableBoys
                        : data.shopOrder?.availableBoys;
                    if (boys && boys.length > 0) {
                      await axios.post(
                        `${serverUrl}/api/order/assign-manual`,
                        {
                          orderId: data._id,
                          shopId,
                          deliveryBoyId: boys[0]._id,
                        },
                        { withCredentials: true },
                      );
                      setAvailableBoys([]);
          ...
          {data.shopOrder?.assignedDeliveryBoy ? (
            <div className="flex items-center justify-between bg-white p-2 rounded border border-orange-100">
              ...
              <p className="text-gray-600 flex items-center gap-1 font-medium">
                <MdPhone size={14} />{" "}
                {data.shopOrder?.assignedDeliveryBoy?.mobile}
              </p>
            </div>
          ) : availableBoys.length > 0 ||
            data.shopOrder?.availableBoys?.length > 0 ? (
            <div className="space-y-2">
              {(availableBoys.length > 0
                ...
                <div
                  key={i}
                  className="flex items-center justify-between border border-orange-200 bg-white p-2 rounded shadow-sm"
                >
                  <div>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        await axios.post(
                          `${serverUrl}/api/order/assign-manual`,
                          { orderId: data._id, shopId, deliveryBoyId: boy._id },
                          { withCredentials: true },
                        );
                        setAvailableBoys([]);
                        window.location.reload();
            ...
            <div className="text-center py-2">
              <p className="text-orange-600 text-xs font-bold mb-2">
                No delivery partners currently online.
              </p>
              <button
                onClick={() =>
                  handleUpdateStatus(data._id, shopId, "out of delivery")
                }
      ...
      <div className="flex justify-between items-center pt-2">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() =>
              dispatch(removeMyOrder({ orderId: data._id, shopId }))
            }
            className="bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-3 py-1.5 rounded-md text-[10px] font-black uppercase tracking-widest"
          >
            Remove
          </button>
