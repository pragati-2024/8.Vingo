...
function UserDashboard() {
  ...
  const shopsLoading = shopInMyCity === null;
  const itemsLoading = itemsInMyCity === null;
  ...
  if (userData === undefined) {
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center bg-transparent">
        ...
        <p className="mt-4 text-white font-black uppercase tracking-widest text-xs">
          Loading Vingo...
        </p>
      </div>
    );
  ...
  const getUniqueItems = (list) => {
    ...
    return Array.from(
      new Map(safe.map((i) => [String(i?._id || i?.id || ""), i])).values(),
    ).filter((i) => i && (i._id || i.id));
  };

  const handleFilterByCategory = (category) => {
    if (category == "All") {
      setUpdatedItemsList(getUniqueItems(itemsInMyCity));
    } else {
      const filteredList = itemsInMyCity?.filter(
        (i) => i.category === category,
      );
      setUpdatedItemsList(getUniqueItems(filteredList));
    }
  };

  useEffect(() => {
    // Extra safety: ensure unique by _id so cards don't repeat.
    const uniqueSafeItems = getUniqueItems(itemsInMyCity);
    setUpdatedItemsList(uniqueSafeItems);
    if (uniqueSafeItems.length > 0) {
      // Deterministic: show only owner-added items (itemsInMyCity is already filtered)
      // and keep the list stable by sorting by latest updates.
      const sorted = [...uniqueSafeItems].sort((a, b) => {
        const aTime = new Date(a?.updatedAt || a?.createdAt || 0).getTime();
        const bTime = new Date(b?.updatedAt || b?.createdAt || 0).getTime();
        return bTime - aTime;
      });

      setTrendingItems(sorted.slice(0, 4));
      setRecommendedItems(sorted.slice(4, 8));
    } else {
      setTrendingItems([]);
      setRecommendedItems([]);
    }
    ...
    <div className="w-full min-h-screen flex flex-col items-center">
      <div className="w-full h-[60vh] md:h-[75vh] relative flex flex-col items-center justify-center text-white overflow-hidden">
        ...
        <div className="z-10 flex flex-col items-center text-center px-4 max-w-4xl">
          ...
          >
            Best food in{" "}
            <span className="text-[#ff4d2d] border-b-4 border-[#ff4d2d]">
              {currentCity}
            </span>{" "}
          ...
          >
            <button
              ...
              className="bg-[#ff4d2d] text-white px-8 py-4 rounded-full font-black uppercase tracking-widest hover:scale-105 transition-transform shadow-xl"
            >
              Order Now
            </button>
            <button
              onClick={handleExploreAll}
              className="bg-white/20 backdrop-blur-md text-white border border-white/30 px-8 py-4 rounded-full font-black uppercase tracking-widest hover:bg-white/30 transition-all"
            >
              Explore
            </button>
          </motion.div>
      ...
      <div className="w-full -mt-20 z-10 pb-20">
        {searchItems && searchItems.length > 0 && (
          <div className="w-full max-w-6xl mx-auto flex flex-col gap-5 items-start p-5 subtle-star-pattern shadow-lg rounded-2xl mb-10">
            ...
            <div className="w-full h-auto flex flex-wrap gap-6 justify-center">
              {Array.from(
                new Map(
                  (Array.isArray(searchItems) ? searchItems : []).map((i) => [
                    String(i?._id || i?.id || ""),
                    i,
                  ]),
                ).values(),
        ...
        <section className="w-full subtle-star-pattern py-14 flex flex-col items-center border-b border-gray-100 bg-white/95 backdrop-blur-[2px]">
          <div className="w-full max-w-300 px-6 lg:px-20 flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight uppercase">
                  Inspiration for your first order
                </h2>
                <p className="text-gray-400 font-medium text-xs mt-1 capitalize">
              ...
              <div className="flex gap-3">
                {showLeftCateButton && (
                  <button
                    ...
                    className="w-9 h-9 rounded-full bg-white shadow-md flex items-center justify-center border border-gray-100 hover:bg-[#ff4d2d] hover:text-white transition-all"
                  >
            ...
            <div
              ref={cateScrollRef}
              className="w-full flex gap-5 overflow-x-scroll no-scrollbar py-2"
            >
        ...
        <section className="w-full py-14 flex flex-col items-center subtle-star-pattern bg-[#f8f9fb]/95 backdrop-blur-[2px]">
          <div className="w-full max-w-300 px-6 lg:px-20 flex flex-col gap-8">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight uppercase">
            ...
            <div
              ref={shopScrollRef}
              className="w-full flex gap-6 overflow-x-scroll no-scrollbar py-4 min-h-50 items-center justify-start"
            >
              {shopsLoading ? (
                  ...
                  >
                    ...
                    <div className="p-5">
                      <div className="flex justify-between items-center mb-1.5">
                        <h3 className="text-lg font-black text-gray-900 truncate group-hover:text-[#ff4d2d] transition-colors uppercase">
                          {shop.name}
                        </h3>
                ...
                <div className="flex flex-col items-center gap-3 py-10">
                  <p className="text-lg font-black text-gray-400 uppercase tracking-tighter">
                    No restaurants found in {currentCity || "this city"} 😔
                  </p>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                    Try searching for "Mathura" or "Delhi"
                  </p>
                </div>
              )}
            </div>
        ...
        <section className="w-full subtle-star-pattern py-14 flex flex-col items-center border-y border-gray-50 bg-white/95 backdrop-blur-[2px]">
          <div className="w-full max-w-300 px-6 lg:px-20 flex flex-col gap-8">
            ...
            <div className="w-full flex flex-wrap gap-x-6 gap-y-10 justify-center md:justify-start min-h-75 items-center">
              {itemsLoading ? (
                <div className="w-full flex flex-col items-center gap-2">
                  <ClipLoader size={30} color="#ff4d2d" />
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                    Curating trending items...
                  </p>
                </div>
                ...
                <div className="w-full flex flex-col items-center gap-3 py-10">
                  <p className="text-lg font-black text-gray-400 uppercase tracking-tighter">
                    No trending items in {currentCity} yet
                  </p>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                    We'll be here soon!
                  </p>
        ...
        <section className="w-full py-20 bg-white/95 backdrop-blur-[2px] border-b border-gray-50 subtle-star-pattern">
          <div className="w-full max-w-300 mx-auto px-6 lg:px-20">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-5xl font-black text-gray-900 tracking-tighter uppercase mb-2">
                How Vingo Works
              </h2>
              <p className="text-[#ff4d2d] font-black uppercase tracking-[0.3em] text-[10px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              ...
              >
                ...
                <p className="text-gray-500 text-sm font-medium">
                  Browse through hundreds of local restaurants in your city.
                </p>
              </motion.div>
              ...
              >
                ...
                <p className="text-gray-500 text-sm font-medium">
                  Add your favorite items to cart and pay securely in seconds.
                </p>
              </motion.div>
              ...
              >
                ...
                <p className="text-gray-500 text-sm font-medium">
                  Track your order in real-time and get it delivered in 30 mins.
                </p>
              </motion.div>
            </div>
          </div>
        ...
        <section
          ref={itemsSectionRef}
          className="w-full subtle-star-pattern py-10 flex flex-col items-center border-t border-gray-50 bg-white/95 backdrop-blur-[2px]"
        >
          <div className="w-full max-w-300 px-6 lg:px-20 flex flex-col gap-8">
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight uppercase">
