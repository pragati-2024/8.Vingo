...
function FoodCard({ data }) {
  ...
  const [realisticRating] = useState(
  ...
  const [discount] = useState(
    Math.random() > 0.6 ? Math.floor(Math.random() * 20) + 10 : null,
  );
    ...
    >
      ...
      <div className="flex-1 flex flex-col p-4">
        <div className="flex justify-between items-start mb-1 gap-1">
          <h1 className="font-black text-gray-800 text-sm leading-tight group-hover:text-[#ff4d2d] transition-colors truncate w-[70%]">
            {data.name}
          </h1>
        ...
        {data.shop?.name && (
          <p className="text-[10px] text-gray-400 font-semibold -mt-0.5 mb-1 truncate">
            {data.shop.name}
          </p>
        )}
        ...
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <div className="flex items-center gap-1 ml-auto">
            ...
            ></span>
            <span
              className={`text-[8px] font-bold uppercase tracking-tight ${isOpen ? "text-green-600" : "text-red-600"}`}
            >
              {isOpen ? "Open" : "Closed"}
            </span>
          </div>
        ...
        <div className="flex items-center justify-between mt-auto gap-2">
          <div className="flex items-center bg-gray-50 rounded-lg p-0.5 border border-gray-100">
            ...
            <span className="w-4 text-center font-bold text-xs text-gray-700">
              {quantity}
            </span>
            <button
              type="button"
